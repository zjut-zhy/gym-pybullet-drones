"""SGE-MambaRL Phase 2 -- Mamba3 RL + backward curriculum + SIL.
...
Phase 2 of the SGE-MambaRL framework:
  ...
  2. SB3 PPO with MambaActorCriticPolicy (Mamba3 feature extractor).
  3. SIL: self-imitation learning from demo + high-return online trajectories.
  4. Alternating resets: backward waypoint / random seed.

Phase 1 (SGE exploration + demo generation) is handled by the go_explore
package and is a prerequisite.
"""
...
from gym_pybullet_drones.our_experiments.sge_mambarl.mamba_policy import (
    MambaActorCriticPolicy,
)
from gym_pybullet_drones.our_experiments.sge_mambarl.sil_buffer import SILBuffer
...
class RobustifyConfig:
    ...
    # ── demo / IO ────────────────────────────────────────────────────
    demo_path: str = "results/sge_mambarl_phase1/demos_best.demo.pkl"
    output_dir: str = "results/sge_mambarl"
    seed: int = 42
    device: str = "auto"
...
class BackwardCurriculumWrapper(gym.Wrapper):
    ...
    Each episode randomly selects (with probability ``curriculum_ratio``):
      ...
      - Normal reset: env.reset() with natural randomness
    """
    ...
    def maybe_shift_backward(self) -> bool:
        """Check and perform backward shift. Called after each PPO update."""
        self._level_iters += 1
...
class SILCallback(BaseCallback):
    """Self-Imitation Learning callback for SB3's PPO with Mamba policy.

    After each PPO rollout:
      1. Do SIL gradient steps using the SIL buffer.
      2. Track online episodes and add high-return ones to the SIL buffer.
      3. Check backward curriculum shift.
      4. Reset Mamba hidden state at episode boundaries.
    """
    ...
    def _on_step(self) -> bool:
        """Track per-step data for SIL online collection + reset Mamba on done."""
        ...
        if obs is not None and actions is not None:
            ...
            if dones is not None and dones[0]:
                ...
                # Reset Mamba hidden state for next episode
    ...
    def _on_rollout_end(self) -> None:
        """After rollout: SIL updates + backward shift check."""
...
def train(cfg: RobustifyConfig) -> None:
    ...
    # -- TensorBoard --
    tb_dir = os.path.join("runs", "sge_mambarl")

    Path(cfg.output_dir).mkdir(parents=True, exist_ok=True)

    # -- SB3 PPO with Mamba policy --
    policy_kwargs = dict(
    ...
    print("\n" + "=" * 70)
    print("Mamba Policy Architecture:")
    print(model.policy)
    print("=" * 70)
    ...
    # -- Print config --
    print("=" * 70)
    print("SGE-MambaRL Phase 2 -- Mamba3 RL + SIL + Backward Curriculum")
    print(f"  Mamba: d_model={cfg.mamba_d_model}, d_state={cfg.mamba_d_state}, "
...
def _parse_args() -> RobustifyConfig:
    parser = argparse.ArgumentParser(
        description="SGE-MambaRL Phase 2 (Mamba3 RL)")
    cfg = RobustifyConfig()
