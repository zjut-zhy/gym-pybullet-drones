...
$M$~mobile target zones $\mathcal{G} = \{G_1, \dots, G_M\}$, and
...
To avoid collisions and remain within the task region, UAVs must maintain safe
distances.
...
between a UAV and an obstacle zone boundary, and
$R_{\mathrm{safe}}^{3} = r_{\mathrm{uav}}$ the minimum safe distance
between a UAV and the task region boundary.
The constraints are
\begin{equation}
  \begin{cases}
    ...
    d_{\mathrm{b},i}^{(t)} \ge R_{\mathrm{safe}}^{3},
    & \forall\, U_i \in \mathcal{U},
  \end{cases}
  \label{eq:safety}
\end{equation}
where
$d_{\mathrm{b},i}^{(t)} = \min\!\bigl(
  \tfrac{L}{2} - |x_i^{(t)}|,\;
  \tfrac{L}{2} - |y_i^{(t)}|
\bigr)$
is the minimum distance from the centre of~$U_i$ to the task region boundary.
...
\begin{enumerate}
  \item \emph{Self-state}
    ...
    the position and velocity of~$U_i$, normalised by the region half-extent~$L/2$
...
combining repulsive penalties that grow as a UAV approaches dangerous regions
with attractive rewards that guide it toward targets.
It decomposes into four components: inter-UAV penalty, obstacle zone penalty,
target reward, and boundary penalty.
...
$\sigma$ is the width of the threat zone surrounding each UAV.
...
$\mathbf{p}_{i,xy} = (x_i, y_i)^{\top}$.

\paragraph{Inter-UAV penalty.}
For a UAV pair $(U_i, U_j)$ with distance
$d = \lVert\mathbf{p}_i - \mathbf{p}_j\rVert$:
\begin{equation}
  r_{\mathrm{uav}} =
  \begin{cases}
    -20 & d \le 2\,r_{\mathrm{uav}}, \\[3pt]
    -\varphi(d,\, 2\,r_{\mathrm{uav}})
    & 2\,r_{\mathrm{uav}} < d < 2\,r_{\mathrm{uav}} + \sigma, \\[3pt]
    0 & \text{otherwise}.
  \end{cases}
  \label{eq:r_uav}
\end{equation}
The penalty is split equally between both UAVs.

\paragraph{Obstacle zone penalty.}
For UAV~$U_i$ and obstacle zone~$T_k$ with planar distance
$d = \lVert\mathbf{p}_{i,xy} - \mathbf{p}_{k,xy}\rVert$, where
$\mathbf{p}_{i,xy}$ and $\mathbf{p}_{k,xy}$ are the $xy$-projections of the
UAV and obstacle zone positions, respectively.
...
Let $d_i^{*} = \min_{m} \lVert\mathbf{p}_{i,xy} - \mathbf{p}_{m,xy}\rVert$
denote the planar distance from~$U_i$ to the nearest target zone centre,
...
\begin{equation}
  ...
  \begin{cases}
    +10
    & d_i^{*} \le R_{\mathrm{cov}}, \\[3pt]
    \varphi(d_i^{*},\, R_{\mathrm{cov}})
    & R_{\mathrm{cov}} < d_i^{*} < R_{\mathrm{cov}} + \sigma, \\[3pt]
    0 & \text{otherwise}.
  \end{cases}
  \label{eq:r_tgt}
\end{equation}
...
Using $d_{\mathrm{b},i}^{(t)}$ defined in~\eqref{eq:safety}, the penalty is
\begin{equation}
  r_{\mathrm{bnd}} =
  \begin{cases}
...
\begin{equation}
  R_{\mathrm{UAV}} = r_{\mathrm{uav}} + r_{\mathrm{obs}}
    + r_{\mathrm{tgt}} + r_{\mathrm{bnd}}.
