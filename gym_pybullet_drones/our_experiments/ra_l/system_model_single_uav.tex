...
composed of a UAV, $M$~mobile ground targets
...
To avoid collisions, the UAV must maintain a safe distance from all obstacle
zone boundaries.
Let $\mathbf{p}_k^{(t)}$ denote the centre of obstacle zone~$T_k$ at
time~$t$, and let $R_{\mathrm{safe}} = r_{\mathrm{uav}} + r_k$ denote the
minimum safe separation.
The constraint is
\begin{equation}
  \lVert\mathbf{p}^{(t)} - \mathbf{p}_k^{(t)}\rVert
    \ge R_{\mathrm{safe}},
  \quad \forall\, T_k \in \mathcal{T}.
  \label{eq:safety}
\end{equation}
In addition, a threat zone of width~$d_{\mathrm{threat}}$
surrounds the UAV; when the distance between the UAV and an obstacle zone
boundary falls within $(R_{\mathrm{safe}},\, R_{\mathrm{safe}} +
d_{\mathrm{threat}})$, a proximity penalty is incurred (see
Section~\ref{subsubsec:reward}).
...
\begin{enumerate}
  \item \emph{Self-state}
    ...
    the position and velocity of the UAV, normalised by the region half-extent
...
The reward function decomposes into two components: obstacle zone penalty and
coverage reward.
...
when the UAV physically contacts the obstacle zone, $\beta$ is the soft
threat-zone penalty coefficient that decays exponentially with distance, and
$d_{\mathrm{threat}}$ is the width of the threat zone.

\paragraph{Coverage reward.}
Each time the UAV covers a target (i.e.\
$I_{\mathrm{cov}}(G_m, t) = 1$), it receives a positive reward
$r_2 = \alpha_{\mathrm{cov}}$, where $\alpha_{\mathrm{cov}}$ is the coverage
reward coefficient.
...
completion ratio;
(ii)~the safety constraint in~\eqref{eq:safety} is violated, i.e.\ the UAV
collides with any obstacle zone; or
