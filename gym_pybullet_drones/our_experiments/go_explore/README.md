...
**纯步数更新**：已有 Cell 发现步数更短的新路径时，更新其 `full_action_sequence`。不使用累积奖励比较（因环境的 shaping reward 会导致蹭奖励的冗长轨迹得分虚高）。采用**懒构建**优化：仅在确认需要创建新 Cell 或步数更优时才构建完整动作序列。
...
从 Archive 中提取**所有成功轨迹**（`n_captured >= target_count`），按步数升序排列（最短路径优先）：
...
- **纯步数更新**：已有 Cell 发现步数更短的新路径时，更新 full_action_sequence
...
| 文件 | 说明 |
|---|---|
| `train.py` | Phase 1：确定性探索主循环（动作回放 + 纯步数更新） |
...
| `archive.py` | Per-Cell Archive 实现（纯步数更新 + Done Cell 过滤 + 持久化） |
