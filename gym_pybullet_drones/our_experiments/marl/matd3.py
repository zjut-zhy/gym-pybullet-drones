# Multi-Agent TD3 (MATD3) with parameter sharing for OurRLAviary.
...
#
# Usage:
#   python matd3.py --num-drones 4 --total-timesteps 500000
import os
import random
import sys
import time
...
class Args:
    ...
    """the scale of policy noise (target policy smoothing)"""
...
if __name__ == "__main__":
    ...
    qf2 = QNetwork(single_obs_dim, single_act_dim).to(device)
    qf1_target = QNetwork(single_obs_dim, single_act_dim).to(device)
    qf2_target = QNetwork(single_obs_dim, single_act_dim).to(device)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                clipped_noise = (torch.randn_like(data_actions) * args.policy_noise).clamp(
                    -args.noise_clip, args.noise_clip
                )
                next_state_actions = (target_actor(data_next_obs) + clipped_noise).clamp(-1, 1)

                # Twin Q: take the min of two target Q-values
